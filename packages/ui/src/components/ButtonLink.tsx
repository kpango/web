...
export const buttonLinkVariants = cva(
  "inline-flex items-center gap-2 font-medium rounded-xl transition-colors",
  {
    variants: {
      variant: {
        primary: "bg-primary hover:bg-primary/90 text-primary-foreground shadow-lg shadow-indigo-900/40",
...
export type ButtonLinkProps = PropsWithChildren<
  { class?: string; href?: string; target?: string; rel?: string; [key: string]: any } & VariantProps<typeof buttonLinkVariants>
>;
...
export const ButtonLink: FC<ButtonLinkProps> = ({
  ...
  return (
    <a class={cn(buttonLinkVariants({ variant, size, className }))} {...props}>
      {children}
    </a>
  );
};
