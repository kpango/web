...
build:
	bun run update-posts
	cd apps/web && bunx tsc -b && bunx vite build
...
docker-build:
	docker build --network=host -t $(DOCKER_IMAGE):$(DOCKER_TAG) .

# Run typecheck and tests inside a disposable container (no image kept)
docker-test:
	docker build --network=host --target build -t $(DOCKER_IMAGE):test .
